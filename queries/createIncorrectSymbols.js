...
module.exports = (challenge_id, lesson_id) => {
  return new Promise((resolve, reject) => {
    const query1 = `INSERT INTO challenges_symbols_incorrect ( lesson_id, challenge_id) VALUES (${lesson_id}, ${challenge_id});`;
    db.query(query1, (err, results, fields) => {
      if (err) {
        reject(err);
      } else {
        const query2 = `SELECT * FROM challenges_symbols_incorrect WHERE id = ${results.insertId};`;
