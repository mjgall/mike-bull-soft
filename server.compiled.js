...
const path = require('path'); // const mysql = require('mysql');
...
app.get('/api/gettest', async (req, res) => {
  db.getConnection((err, connection) => {
    ...
    connection.query(`SELECT * FROM users WHERE id = ${req.body.id}`, (err, results, fields) => {
      if (err) {
...
app.get('/api/anothertest', async (req, res) => {
  ...
  db.getConnection((err, connection) => {
    if (err) throw err;
    connection.query(`SELECT * FROM users WHERE id = ${id}`, (err, users, fields) => {
      if (err) throw err;
      res.json(users[0].id);
    });
    connection.release();
